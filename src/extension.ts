...
export function activate(context: vscode.ExtensionContext) {
  ...
  const treeDataProvider = new ReactComponentProfiler(
    rootPath as string,
    treeBuilder,
    context
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "project-component-tree.refreshEntry",
      () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "project-component-tree.onlyComponentFile",
      function () {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      "project-component-tree.onlyComponent",
      function () {
    ...
    provView.onDidChangeSelection((e) => {
      e.selection.forEach((node) => {
        if (node.type === "placeholder" && node.label.startsWith("from:")) {
          ...
          vscode.workspace
            .openTextDocument(nodeFilePath.trim())
            .then((document) => vscode.window.showTextDocument(document));
        } else if (node.type === "component" && (node.count ?? 0) < 1) {
          vscode.workspace
            .openTextDocument(node.path.trim())
            .then((document) => vscode.window.showTextDocument(document));
        }
      });
    })
  ...
  function setButtonsAndContext() {
    ...
    context.workspaceState.update(
      "srcFolder",
      vscode.workspace
        .getConfiguration("react-component-profiler")
        .get("srcFolder") ?? "src"
    );
    const isFileOnly = context.workspaceState.get("showFileOnly", false);
    const isComponentOnly = context.workspaceState.get(
      "showComponentOnly",
      false
    );
