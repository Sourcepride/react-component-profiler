...
export class ComponentTreeBuilder {
  ...
  public createTreeDataStructures(workspacePath: string) {
    if (!(workspacePath && this.isReactProject(workspacePath))) {
      return;
    }
    ...
    /*
        ...
        */
    const rootNode = Node.createNewNode(
      nodePath.join(workspacePath, this.srcFolder)
    );
  ...
  private depthFirstFolderTreeTraversal(node: Node): number {
    ...
    for (let file of subFileArray) {
      const filePath = nodePath.join(node.path, file);
      if (!this.isReactFileOrFolder(filePath)) {
        continue;
      }
      ...
      if (subNodeComponentCount === 0) {
        node.children.pop();
      } else {
        componentCount += subNodeComponentCount;
      }
    }
  ...
  private isReactFileOrFolder(file: string): boolean {
    ...
    if (stat.isDirectory()) {
      ...
      const ext = nodePath.parse(file).ext;
  ...
  private extraAllInformation(file: string): number {
    const fileExtension = nodePath.parse(file).ext.replace(/\./, "");
    ...
    traverse(ast, {
      ...
      ImportDeclaration(path) {
        ...
        const impResolver = new ImportPathResolver(
          workspacePath,
          file,
          source,
          srcFolder
        ...
        path.node.specifiers.forEach((specifier) => {
          if (
            ...
            t.isImportDefaultSpecifier(specifier) &&
            specifier.local.name.match(/^[A-Z][a-zA-Z0-9]*$/)
          ) {
            addImportedComponentToHashMap(
              file,
              resolvedSource,
  ...
  private findIfScopeReturnsJSX(
    inputBody: t.BlockStatement | t.Expression
  ): boolean {
    if (!t.isBlockStatement(inputBody)) {
      return false;
    }
    ...
    for (let element of inputBody.body) {
      if (!t.isReturnStatement(element)) {
        continue;
      }
      if (
        t.isJSXElement(element.argument) ||
        t.isJSXFragment(element.argument)
      ) {
        returnVal = true;
      }
    }
  ...
  private addComponentsToHashMap(file: string, componentNames: string[]) {
    const identifier = nodePath.parse(file).name;
    const extension = nodePath.parse(file).ext.replace(/\./, "");

    if (_COMPONENT_MAP.has(identifier)) {
      const initialValue: ComponentFileRecordType =
        _COMPONENT_MAP.get(identifier);

      _COMPONENT_MAP.set(identifier, {
  ...
  private getIntialOrDefault(
    name: string,
    initialValue: ComponentFileRecordType
  ) {
  ...
  private addImportedComponentToHashMap(
    file: string,
    ...
    if (_COMPONENT_MAP.has(fullSourcePath)) {
      const initialValue: ComponentFileRecordType =
        _COMPONENT_MAP.get(fullSourcePath);
      const foundComp = initialValue.components.find(
      ...
      _COMPONENT_MAP.set(fullSourcePath, {
        components: [
          ...(foundComp
            ? initialValue.components.map((item) => {
  ...
  private addHookRecordForComponent(
    file: string,
    ...
    if (_COMPONENT_MAP.has(identifier)) {
      const initialValue: ComponentFileRecordType =
        _COMPONENT_MAP.get(identifier);
      const foundHook = initialValue.hooks.find(
      ...
      if (!foundHook) {
        _COMPONENT_MAP.set(identifier, {
          ...
          hooks: [
            ...initialValue.hooks,
            { name: hookName, source: hookSourcePath },
          ],
          extension: initialValue.extension,
        });
      }
  ...
  public isReactProject(projectRootPath: string): boolean {
    const srcPath: string = nodePath.join(projectRootPath, this.srcFolder);
    const packageDotJsonPath: string = nodePath.join(
      projectRootPath,
      "package.json"
    );
    if (!(this.pathExists(srcPath) && this.pathExists(packageDotJsonPath))) {
      return false;
    }
