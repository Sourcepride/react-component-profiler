...
import {
  ComponentFileRecordType,
  ComponentRecordType,
  ComponentTreeBuilderI,
  entityType,
  node,
} from "./types";
...
export class ReactComponentProfiler
  implements vscode.TreeDataProvider<NodeInfo>
{
  private _onDidChangeTreeData: vscode.EventEmitter<
    NodeInfo | undefined | null | void
  > = new vscode.EventEmitter<NodeInfo | undefined | null | void>();
  ...
  constructor(
    ...
    private componentFinder: ComponentTreeBuilder,
    private readonly context_: vscode.ExtensionContext
  ) {
    this.componentFinder.setSrcFolder(
      context_.workspaceState.get("srcFolder", "src")
    );
  ...
  getChildren(element?: NodeInfo): Thenable<NodeInfo[]> {
    ...
    if (element) {
      ...
      switch (element.type) {
        ...
        case "component": {
          response = this.getComponentNodes(
            element.path,
            element.label,
            this.componentFinder
          );
        ...
        case "placeholder": {
          response =
            element.label === "hooks"
              ? this.getHooksNodes(element.path, this.componentFinder)
              : [];
          break;
        }
        default:
      ...
      const showFileOnly = this.context_.workspaceState.get(
        "showFileOnly",
        false
      );
      const showComponentOnly = this.context_.workspaceState.get(
        "showComponentOnly",
        false
      );
      if (showFileOnly && !showComponentOnly) {
        const componentFilePaths = this.componentFinder
          .getComponetsStore()
          .keys();
            ...
            .filter(
              (key) =>
                !!(
                  ...
                  this.componentFinder.getComponents(key.split(".")[0])
                    ?.components
                )
            )
            .map((val: string) => {
              const componentInfo = this.componentFinder.getComponents(
                val.split(".")[0]
              ) as ComponentFileRecordType;
              return new Node(
                [],
                componentInfo.components.length,
                "componentFile",
                componentInfo.extension &&
                !val.endsWith(`${componentInfo.extension}`)
                  ? `${val}.${componentInfo.extension}`
                  : val
              );
            }),
          0,
        ...
        return Promise.resolve(
          this.getFolderNodes(fakeFolderNode, this.componentFinder)
        );
        ...
        Array.from(componentFilePaths)
          ...
          .forEach((val: string) => {
            allComponents = [
              ...allComponents,
              ...(
                this.componentFinder.getComponents(
                  val.split(".")[0]
                ) as ComponentFileRecordType
              ).components.map((comp) => {
      ...
      return Promise.resolve(
        this.getFolderNodes(
          this.componentFinder.getRootNode(),
          this.componentFinder
        )
      );
    }
  ...
  private getFolderNodes(
    node: node | undefined,
    componentFinder: ComponentTreeBuilderI
  ) {
    if (node === undefined) {
      return [];
    }
    ...
    if (node.children.length > 0) {
      return node.children.map((child) => {
        if (child.type === "folder") {
          ...
          const components = componentFinder.getComponents(
            child.path.split(".")[0]
          );
          if (components) {
            return new NodeInfo(
  ...
  private getComponentNodes(
    compPath: string,
    componentName: string,
    componentFinder: ComponentTreeBuilderI
  ) {
    const components = componentFinder.getComponents(compPath.split(".")[0]);
    const componentName_ =
      componentName.split(".").length > 0
        ? componentName.split(".")[0]
        : componentName;

    if (components) {
      const compInfo = components.components.find(
        (comp) => comp.name === componentName_
      );
      if (compInfo) {
...
class NodeInfo extends vscode.TreeItem {
  constructor(
    ...
    if (["folder", "file", "component"].includes(this.type)) {
      this.description = `${this.label} ${type === "component" ? "CU" : "C"}:(${
        this.count ?? 0
      })`;
    } else {
      this.description = `${this.label} ${
        type === "placeholder" && count
          ? `${this.label.substring(0, 1)}:(${count})`
          : ""
      }`;
    }
    ...
    this.iconPath =
      typeof iconMap[type] === "string"
        ? new vscode.ThemeIcon(iconMap[type])
        : iconMap[type];
